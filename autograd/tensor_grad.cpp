...
static Var::Ptr _##opName##_child_grad(Var::Ptr const &self, int i)

CHILD_GRAD_FUNC_DEF(add){return self->gradient_node;}
CHILD_GRAD_FUNC_DEF(addConst){return self->gradient_node;}
CHILD_GRAD_FUNC_DEF(sub){return i ? self->gradient_node*(-1): self->gradient_node;}
CHILD_GRAD_FUNC_DEF(subConst){return self->gradient_node;}
CHILD_GRAD_FUNC_DEF(rsubConst){return self->gradient_node*(-1);}
CHILD_GRAD_FUNC_DEF(mul){
    if(i==0)
        return self->gradient_node*(self->getChild(1));
    else return self->gradient_node*(self->getChild(0));
}
CHILD_GRAD_FUNC_DEF(mulConst){return self->gradient_node*(*self->getConstAttr());}
...
CHILD_GRAD_FUNC_DEF(exp){
    return self->gradient_node*(self);
}
...
typedef Var::Ptr (*TChildGradFunc)(Var::Ptr const &self, int i);
...
Var::Ptr Var::gradOfChild(Ptr const &self, int i)
{
...
void Var::compute_gradient(Ptr const &self){
    // 先计算逆拓扑序
    std::vector<Var::Ptr> topo_order{};
    std::unordered_set<Var*> visited;
    dfs(self, topo_order, visited);
    self->gradient_node = make(1., false);
    // 按逆拓扑序访问
    for(auto iter = topo_order.rbegin(); iter!=topo_order.rend(); ++iter){
        ...
        for(int i=0; i<OP_NODE_CNT[node->op_type]; ++i){
            ...
            child->gradient_node = child->gradient_node + (gradOfChild(node, i));
        }
    }
}
